...
for SERVICE in "${LEGACY_ECS_SERVICES[@]}"; do
  ...
  if aws ecs describe-services --cluster ${CLUSTER_NAME} --services ${SERVICE} --region ${AWS_REGION} ${AWS_PROFILE_ARG} --query 'services[0].serviceName' --output text 2>/dev/null | grep -q ${SERVICE}; then
    ...
    aws ecs update-service --cluster ${CLUSTER_NAME} --service ${SERVICE} --desired-count 0 --region ${AWS_REGION} ${AWS_PROFILE_ARG}
    
    # Wait for tasks to stop
    echo "⏳ Waiting for tasks to stop..."
    aws ecs wait services-stable --cluster ${CLUSTER_NAME} --services ${SERVICE} --region ${AWS_REGION} ${AWS_PROFILE_ARG}
    
    # Delete the service
    echo "🗑️  Deleting legacy service..."
    aws ecs delete-service --cluster ${CLUSTER_NAME} --service ${SERVICE} --region ${AWS_REGION} ${AWS_PROFILE_ARG}
...
LEGACY_APPRUNNER_SERVICES=$(aws apprunner list-services --region ${AWS_REGION} ${AWS_PROFILE_ARG} --query "ServiceSummaryList[?contains(ServiceName, 'v2')].{Name:ServiceName,Arn:ServiceArn}" --output table 2>/dev/null || echo "")
...
aws ecs list-task-definitions --family-prefix glossary-task-v2 --status ACTIVE --region ${AWS_REGION} ${AWS_PROFILE_ARG} --query 'taskDefinitionArns[]' --output table 2>/dev/null || echo "None found"
